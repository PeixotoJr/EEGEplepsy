...
%% Define variables MFCC
   ...
   fs=256;              % frequencia de amostragem   
%% Inicia Variveis
 MELs.MFCCs=[];
 MELs.FBEs=[];
 MELs.frames=[];
 ...
 %%
 for i=1:length(D.Len) 
     Paciente = int2charjp(i)
     for j=1:length(D.Arq2{i})
     File = int2charjp(D.Arq2{i}(j))
     ...
     % Feature extraction (feature vectors as columns)
        [MELs.MFCCs{i,k}, MELs.FBEs{i,k}, frames] = ...
        mfcc(speech, fs, Tw, Ts, alpha, @hamming, [LF HF], M, C+1, L );
        if mod(Seg,2)==0
            MELs.MFCCs{i,k}(:,end+1)=MELs.MFCCs{i,k}(:,end);
            MELs.FBEs{i,k}(:,end+1)=MELs.FBEs{i,k}(:,end);
        end
